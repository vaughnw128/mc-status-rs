...
pub async fn status(ctx: Context<'_>) -> Result<(), Error> {
    ...
    .await?
    {
        Ok((ping, server_information)) => {
            let (player_count, player_names): (usize, String) =
                match server_information.players.sample {
                    Some(s) => {
